...
from app.schemas.user import UserRegisterSchema,UserTokenDataSchema
...
def register(new_user:UserRegisterSchema,
...
def login(form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    if not user_data:
        raise InvalidDataError("Неверный email или пароль")
    new_token = create_token(user_data,session)
    return {"access_token": new_token,
            "token_type": "bearer"}

@router.post("/logout")
def logout(b_token:str = Depends(user_auth),
           session:Session = Depends(get_session)):
